...
//TODO: enable convenient logout!
export default function Enter(props) {
  const { user, username } = useContext(UserContext);
  return (
    <main>
      <Metatags title="Log in" />
      {user ? !username ? <UsernameForm /> : <LogoutButton /> : <LoginButton />}
    </main>
  );
}
...
const UsernameMessage = ({ username, isValid, isLoading }) => {
  if (isLoading) {
    ...
    return (
      <p>
        Usernames must be between 3 and 15 characters long and may only contain

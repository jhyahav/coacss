...
export default function PostForm({ postDefaults, postRef, preview }) {
  ...
  const updatePost = async ({ content, published }) => {
    ...
    const redirect = published
      ? `/${postDefaults.username}/${postDefaults.slug}`
      : "/admin";
      ...
      <div className={preview ? "hidden" : styles.controls}>
        ...
        <textarea
          ...
          {...register("content", {
            required: {
              value: true,
              message: "Your post needs to have some content!",
            },
            maxLength: { value: 15000, message: "Content is too long." },
            minLength: {
              value: 10,
              message: "Your post must be at least 10 characters long.",
            },
          })}
        ></textarea>
        {errors.content && (
